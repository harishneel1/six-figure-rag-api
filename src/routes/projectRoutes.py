...
async def get_projects(current_user_clerk_id: str = Depends(get_current_user_clerk_id)):
    ...
    * 3. Return projects data
    """
    try:
        logger.info("fetching_projects", clerk_id=current_user_clerk_id)
        ...
        logger.info("projects_retrieved", clerk_id=current_user_clerk_id, project_count=len(projects_query_result.data or []))
        return {
        ...
        logger.error("projects_fetch_failed", clerk_id=current_user_clerk_id, error=str(e), exc_info=True)
        raise HTTPException(
            status_code=500,
...
async def create_project(
    ...
    * 6. Return newly created project data
    ...
    try:
        print("jkl")
        logger.info("creating_project", name=project_data.name, clerk_id=current_user_clerk_id)
        ...
        if not project_creation_result.data:
            logger.error("project_creation_failed", name=project_data.name, clerk_id=current_user_clerk_id, reason="no_data_returned")
            raise HTTPException(
                status_code=422,
        ...
        logger.info("project_created", project_id=newly_created_project["id"], name=project_data.name)
        ...
        if not project_settings_creation_result.data:
            logger.error("project_settings_creation_failed", project_id=newly_created_project["id"], reason="no_data_returned")
        ...
        logger.info("project_created_successfully", project_id=newly_created_project["id"], name=project_data.name)
        return {
        ...
        logger.error("project_creation_error", name=project_data.name, clerk_id=current_user_clerk_id, error=str(e), exc_info=True)
        raise HTTPException(
            status_code=500,
...
async def delete_project(
    ...
    * 5. Return successfully deleted project data
    """
    try:
        logger.info("deleting_project", project_id=project_id, clerk_id=current_user_clerk_id)
        ...
        if not project_ownership_verification_result.data:
            logger.warning("project_not_found_or_unauthorized", project_id=project_id, clerk_id=current_user_clerk_id)
            raise HTTPException(
        ...
        if not project_deletion_result.data:
            logger.error("project_deletion_failed", project_id=project_id, reason="no_data_returned")
            raise HTTPException(
        ...
        logger.info("project_deleted_successfully", project_id=project_id, clerk_id=current_user_clerk_id)
        return {
        ...
        logger.error("project_deletion_error", project_id=project_id, clerk_id=current_user_clerk_id, error=str(e), exc_info=True)
        raise HTTPException(
            status_code=500,
...
async def get_project(
    ...
    * 3. Return project data
    """
    try:
        logger.info("fetching_project", project_id=project_id, clerk_id=current_user_clerk_id)
        project_result = (
            supabase.table("projects")
            .select("*")
            .eq("id", project_id)
        ...
        if not project_result.data:
            logger.warning("project_not_found", project_id=project_id, clerk_id=current_user_clerk_id)
            raise HTTPException(
                status_code=404,
        ...
        logger.error("project_retrieval_error", project_id=project_id, error=str(e), exc_info=True)
        raise HTTPException(
            status_code=500,
...
async def get_project_chats(
    ...
    * 3. Return project chats data
    """
    try:
        logger.info("fetching_project_chats", project_id=project_id, clerk_id=current_user_clerk_id)
        ...
        logger.info("project_chats_retrieved", project_id=project_id, chat_count=len(project_chats_result.data or []))
        return {
        ...
        logger.error("project_chats_retrieval_error", project_id=project_id, error=str(e), exc_info=True)
        raise HTTPException(
            status_code=500,
...
async def get_project_settings(
    ...
    * 4. Return project settings data
    """
    try:
        logger.info("fetching_project_settings", project_id=project_id, clerk_id=current_user_clerk_id)
        ...
        if not project_settings_result.data:
            logger.warning("project_settings_not_found", project_id=project_id)
            raise HTTPException(
                status_code=404,
                detail="Project settings not found or you don't have permission to access it",
            )

        logger.info("project_settings_retrieved", project_id=project_id, rag_strategy=project_settings_result.data[0].get("rag_strategy"))
        return {
        ...
        logger.error("project_settings_retrieval_error", project_id=project_id, error=str(e), exc_info=True)
        raise HTTPException(
            status_code=500,
...
async def update_project_settings(
    ...
    * 6. Return successfully updated project settings data
    """
    try:
        logger.info("updating_project_settings", project_id=project_id, clerk_id=current_user_clerk_id, rag_strategy=settings.rag_strategy)
        project_ownership_verification_result = (
            supabase.table("projects")
            .select("id")
            .eq("id", project_id)
        ...
        if not project_ownership_verification_result.data:
            logger.warning("project_not_found_for_settings_update", project_id=project_id, clerk_id=current_user_clerk_id)
            raise HTTPException(
                status_code=404,
        ...
        if not project_settings_ownership_verification_result.data:
            logger.warning("project_settings_not_found_for_update", project_id=project_id)
            raise HTTPException(
                status_code=404,
        ...
        if not project_settings_update_result.data:
            logger.error("project_settings_update_failed", project_id=project_id, reason="no_data_returned")
            raise HTTPException(
                status_code=422, detail="Failed to update project settings"
            )

        logger.info("project_settings_updated_successfully", project_id=project_id, rag_strategy=settings.rag_strategy)
        return {
        ...
        logger.error("project_settings_update_error", project_id=project_id, error=str(e), exc_info=True)
        raise HTTPException(
            status_code=500,
...
async def send_message(
    ...
    """
    try:
        logger.info("sending_message", project_id=project_id, chat_id=chat_id, message_length=len(message.content))
        # Step 1 : Insert the message into the database.
        ...
        try:
            ...
            logger.warning("settings_retrieval_failed_defaulting_to_simple", project_id=project_id, error=str(e))
            agent_type = "simple"

        logger.info("agent_type_determined", project_id=project_id, agent_type=agent_type)
        ...
        logger.info("invoking_agent", project_id=project_id, chat_id=chat_id, agent_type=agent_type)
        ...
        logger.error("send_message_error", project_id=project_id, chat_id=chat_id, error=str(e), exc_info=True)
        raise HTTPException(
            status_code=500,
